...
function App() {
    ...
    return (
      <div style={{ padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          <h1>RoyalRaptorPOS</h1>
          <button onClick={() => setView('pos')}>Go to POS</button>
        </div>
        <ProductsAdmin />
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pt-16 pb-6">
      <Topbar
        showSearch
        searchValue={q}
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-3">
            {products.map((p) => (
              <div key={p._id} className="border border-gray-200 rounded-lg p-3 w-[180px]">
                <div className="font-semibold">{p.name}</div>
                <div className="text-xs text-gray-500">{p.sku}</div>
                <div className="mt-2">${p.price.toFixed(2)}</div>
                <button className="mt-2 btn" onClick={() => addToCart(p)}>Add</button>
              </div>
            ))}
          </div>
        </div>
        <div className="w-full max-w-sm">
          ...
          {cart.length === 0 ? (
            ...
            <div>
              {cart.map((i) => (
                <div key={i.productId} className="flex items-center justify-between gap-2 py-2 border-b border-gray-100">
                  <div>
                    <div className="font-semibold">{i.name}</div>
                    <div className="text-xs text-gray-500">${i.price.toFixed(2)}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button className="btn" onClick={() => updateQty(i.productId, -1)}>-</button>
              ...
              <button className="btn mt-3" disabled={placing || cart.length === 0} onClick={checkout}>
                {placing ? 'Placing…' : 'Checkout'}
              </button>
              <SalePanel cart={cart} setCart={setCart} onPlaced={(sale) => alert(`Sale ${sale._id} placed`)}/>
              {lastOrderId && (
