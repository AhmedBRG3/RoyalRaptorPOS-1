...
function App() {
  ...
  const handleKeyDown = (e) => {
    ...
    // If there's a long pause, reset buffer
    if (now - lastTime > 50) {
      buffer = "";
    }
    lastTime = now;
    ...
    if (e.key === "Enter") {
      if (buffer) {
        ...
        // You can trigger search, fetch product, or add to cart here
      }
      buffer = "";
    ...
    <div className="flex flex-col mt-24 mx-8">
      ...
      <div className="flex gap-4">
        ...
        <div className=" w-full max-w-sm">
          ...
          {cart.length === 0 ? <p>No items in cart</p> : <Cart cart={cart} setCart={setCart} />}
        </div>
      </div>
    </div>
  );
