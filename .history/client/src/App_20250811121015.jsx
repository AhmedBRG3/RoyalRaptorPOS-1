import { useEffect, useMemo, useState, useRefd } from "react";
import "./App.css";
...
function App() {
  ...
  useEffect(() => {
    onSearchRef.current = async (e, directValue) => {
      e?.preventDefault?.();
      try {
        setLoading(true);
  ...
  useEffect(() => {
    ...
    const processBuffer = () => {
      ...
      if (isScannerBurst) {
        ...
        // Update controlled search input and run search with the full buffered value
        setQ(buf); // update UI immediately
        // call the search handler via ref so we don't rely on a fresh closure
        if (typeof onSearchRef.current === "function") {
          onSearchRef.current(null, buf);
        }
      } else {
