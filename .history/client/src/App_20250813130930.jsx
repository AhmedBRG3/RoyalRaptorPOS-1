...
function App() {
  ...
  console.error("THIS APP IS MADE BY OMAR ALHUSSENI, IG @omaralhusseni, omar.alhusseni@icloud.com")
  console.warn("THIS APP IS MADE BY OMAR ALHUSSENI, IG @omaralhusseni, omar.alhusseni@icloud.com")
  console.log("THIS APP IS MADE BY OMAR ALHUSSENI, IG @omaralhusseni, omar.alhusseni@icloud.com")
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  const addToCart = (p) => {
    ...
    // Determine if it's a new line item before updating state to avoid duplicate alerts in StrictMode
    const existing = cart.find((i) => i._id === p._id);
    if (!existing && (p.quantity ?? 0) < 5) {
      alert(`Low stock: Only ${p.quantity ?? 0} left for ${p.name}`);
    }
    setCart((prev) => {
      if (existing) {
        return prev.map((i) =>
          i._d === p._id ? { ...i, quantity: i.quantity + 1 } : i
        );
      }
      return [
        ...prev,
        { _id: p._id, name: p.name, price: p.price, quantity: 1 },
      ];
    });
  };
      ...
      <div className="flex gap-4">
        ...
        <div className=" w-full max-w-sm">
          ...
          {cart.length === 0 ? (
            ...
            <Cart cart={cart} setCart={setCart} onSaleCompleted={refreshProducts} placeSaleRef={placeSaleRef} />
          )}
        </div>
      </div>
    </div>
