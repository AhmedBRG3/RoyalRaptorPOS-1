    ...
    if (keystrokeCount > 6) {
      ...
      if (e.key === "Enter") {
        if (buffer.trim() !== "") {
          ...
          // Update the search bar UI immediately
          onSearchChange({ target: { value: buffer } });

          // Trigger the search right away with the buffered value
          if (typeof onSearchSubmit === "function") {
            onSearchSubmit({ preventDefault: () => {} }, buffer);
          }
        }

        // Reset AFTER using the data
        buffer = "";
        keystrokeCount = 0;
        clearTimeout(timer);
        timer = null;
    ...
    <div className="fixed top-0 inset-x-0 z-50 bg-black backdrop-blur-md border-b border-gray-200 shadow-sm">
      <div className="flex items-center justify-between px-6 py-3">
          ...
          {showSearch && (
            ...
            >
              ...
              <input
                ...
                className="h-10 pl-10 pr-4 text-sm rounded-full focus:outline-none bg-transparent w-full"
                style={{ minWidth: 180 }}
              />
              <button
                type="submit"
        ...
        <div className="flex items-center gap-2 text-white">
          ...
          <NavButton href="/" icon={<Home className="w-4 h-4 mr-1" />}>Home</NavButton>
          <NavButton href="/sales" icon={<ShoppingCart className="w-4 h-4 mr-1" />}>Sales</NavButton>

          {isAdmin && (
            <>
              <NavButton href="/users" icon={<Users className="w-4 h-4 mr-1" />}>Users</NavButton>
              <NavButton href="/sessions" icon={<UserCircle2 className="w-4 h-4 mr-1" />}>Sessions</NavButton>
              <NavButton href="/manageProducts" icon={<Package className="w-4 h-4 mr-1" />}>Products</NavButton>
            </>
          )}
