...
export default function Topbar({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      // Add this key to the buffer unless it's Enter
      if (e.key !== "Enter") {
        buffer += e.key;
      }
      ...
      if (keystrokeCount > 6) {
        ...
        // If Enter ends the burst, run search with previous keys
        if (e.key === "Enter" && buffer.trim() !== "") {
          console.log("Triggering search with:", buffer);
          setQ(buffer); // set the search input value
          onSearch(); // run your existing search
          buffer = "";
          keystrokeCount = 0;
          clearTimeout(timer);
          timer = null;
