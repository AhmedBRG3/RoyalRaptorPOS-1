...
async function generateBarcodeBuffer(code, barcodeType) {
  const finalWidthPx = mmToPixels(LABEL_WIDTH_MM); // 40mm
  const finalHeightPx = mmToPixels(LABEL_HEIGHT_MM); // 30mm

  // Calculate barcode dimensions (90% of label width for bars, 80% of height for bars, 10% for text)
  const BAR_WIDTH = Math.round(finalWidthPx * 0.8); // Increased barcode width
  const BAR_HEIGHT = Math.round(finalHeightPx * 0.6); // Increased barcode height
  const TEXT_SIZE = Math.round(finalHeightPx * 0.04); // Reduced text size

  console.log(
    `Generating barcode: ${finalWidthPx}x${finalHeightPx}px (label: ${LABEL_WIDTH_MM}x${LABEL_HEIGHT_MM}mm)`
  );
  console.log(
    `Barcode dimensions: ${BAR_WIDTH}x${BAR_HEIGHT}px, text size: ${TEXT_SIZE}px`
  );
  ...
  const rawBuffer = await bwipjs.toBuffer({
    ...
    text: code,
    scale: 3, // Increase scale for sharper bars
    ...
    paddingwidth: 0, // Reduced padding to minimize white space
    paddingheight: 0, // Reduced padding to minimize white space
  });
