...
async function generateBarcodeBuffer(code, barcodeType) {
  ...
  const rawBuffer = await bwipjs.toBuffer({
    ...
    text: code,
    scaleX: 3, // Bar thickness
    scaleY: 3, // Vertical scale for bars
    height: pxHeight / 3, // Make bars tall enough
    includetext: true,
    textxalign: "center",
    backgroundcolor: "#FFFFFF", // White background (for bwipjs internal rendering)
    paddingwidth: 4,
    paddingheight: 4
  });

  // Flatten against white to avoid transparency turning black
