  ...
  try {
    ...
    switch (platform) {
      case "win32":
        ...
        const winOutput = execSync('wmic printer get name /format:csv', { encoding: 'utf8' });
        printers = winOutput
          .split('\n')
          .map(line => line.trim())
        ...
        if (printers.length === 0) {
          try {
            const psOutput = execSync('powershell "Get-Printer | Where-Object {$_.Name -like \'*xprint*\'} | Select-Object -ExpandProperty Name"', { encoding: 'utf8' });
            printers = psOutput
              .split('\n')
              .map(line => line.trim())
              .filter(name => name && name.toLowerCase().includes('xprint'));
          } catch (psError) {
            console.warn('PowerShell printer detection failed:', psError.message);
          }
        }
        break;
...
function generateTSPL(code, barcodeType = 'CODE128', copies = 1) {
  let barcodeCommand;
...
router.post("/", (req, res) => {
  ...
  try {
    ...
    if (platform === "win32") {
      ...
      // Windows: Try multiple approaches for better compatibility
      const escapedPrinterName = printerName.replace(/"/g, '""');
      
      // Method 1: Direct copy to printer share
      const copyCmd = `copy /b "${tmpFile}" "\\\\localhost\\${escapedPrinterName}"`;
      // Method 2: Print command with exact name
      const printCmd1 = `print /D:"${printerName}" "${tmpFile}"`;
      // Method 3: Print command with escaped name  
      const printCmd2 = `print /D:"${escapedPrinterName}" "${tmpFile}"`;
      
      printCmd = `(${copyCmd}) || (${printCmd1}) || (${printCmd2})`;
    } else {
...
router.get("/debug-windows", (req, res) => {
  ...
  try {
    ...
    const wmicOutput = execSync('wmic printer get name,drivername,portname /format:csv', { encoding: 'utf8' });
    const psOutput = execSync('powershell "Get-Printer | Select-Object Name,DriverName,PortName | ConvertTo-Json"', { encoding: 'utf8' });
    ...
    const xprinters = parsedPS.filter(p => p.Name && p.Name.toLowerCase().includes('xprint'));
