...
try {
  ...
  console.warn('Printer driver not found. Install with: npm install printer');
  printerDriver = null;
}
...
function findXPrinter() {
  ...
  try {
    ...
    // Check if driver is required but missing
    if (!printerDriver && (os.platform() === 'darwin' || os.platform() === 'linux')) {
      return res.status(500).json({ 
        error: "Printer driver not installed",
        solution: "Run: npm install printer",
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    // Create printer configuration based on platform
    const printerConfig = {
    ...
    // Add driver only if available (required for macOS/Linux)
    if (printerDriver) {
      printerConfig.driver = printerDriver;
    }
