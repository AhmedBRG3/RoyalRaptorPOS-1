...
async function generateBarcodeBuffer(code, barcodeType) {
  ...
  const BAR_WIDTH = Math.round(finalWidthPx * 0.7); // Increased barcode width
  const BAR_HEIGHT = Math.round(finalHeightPx * 0.2); // Increased barcode height
  const TEXT_SIZE = Math.round(finalHeightPx * 0.03); // Reduced text size
