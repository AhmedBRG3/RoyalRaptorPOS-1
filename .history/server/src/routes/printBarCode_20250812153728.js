...
async function generateBarcodeBuffer(code, barcodeType) {
  ...
  const pxHeight = mmToPixels(LABEL_HEIGHT_MM) * 0.6; // Leave some white space

  return bwipjs.toBuffer({
    bcid: barcodeType.toLowerCase(),
    text: code,
    scale: 1, // We'll control size via width/height
    height: pxHeight / 3, // Adjust so it fills area proportionally
    width: pxWidth,
    includetext: true,
    textxalign: "center",
...
async function createBarcodePDF(code, barcodeType, pdfPath) {
  ...
  // Rotate BEFORE inserting into PDF
