...
async function generateBarcodeBuffer(code, barcodeType) {
  ...
  // Calculate barcode dimensions (80% of label width for bars, 15% of height for text)
  const BAR_WIDTH = Math.round(finalWidthPx * 0.8);  // Barcode width
  const BAR_HEIGHT = Math.round(finalHeightPx * 0.6); // Barcode height
  const TEXT_SIZE = Math.round(finalHeightPx * 0.15); // Text size
