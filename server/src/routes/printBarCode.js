...
/**
 * Parse a data URL (e.g. data:image/png;base64,...) into { mimeType, buffer }
 */
function parseDataUrlToBuffer(dataUrl) {
  if (typeof dataUrl !== "string") throw new Error("Image payload must be a string data URL");
...
/**
 * Normalize any input image (PNG/JPEG/SVG) to exact label size PNG buffer
 */
async function normalizeImageToLabelPng(imageBuffer) {
  const finalWidthPx = mmToPixels(LABEL_WIDTH_MM);
  const finalHeightPx = mmToPixels(LABEL_HEIGHT_MM);
...
/**
 * Create PDF file for Windows printing from a provided image buffer
 */
async function createImagePDF(imageBuffer, pdfPath) {
  const widthPts = mmToPoints(LABEL_WIDTH_MM);
  const heightPts = mmToPoints(LABEL_HEIGHT_MM);
...
router.post("/", async (req, res) => {
  const { code, barcodeType = "code128", copies = 1, image } = req.body;
  const platform = os.platform();

  // If image data URL is provided, prefer it; otherwise fall back to generating from code
  const useClientImage = Boolean(image);
  ...
  try {
    ...
    if (platform === "win32") {
      ...
      if (useClientImage) {
        const { buffer } = parseDataUrlToBuffer(image);
        const normalized = await normalizeImageToLabelPng(buffer);
        await createImagePDF(normalized, pdfPath);
      } else {
    ...
    res.json({ 
      ...
      mode: useClientImage ? "client-image" : "server-generated",
    });
